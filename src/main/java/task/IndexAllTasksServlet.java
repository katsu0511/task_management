...
/**
 * Servlet implementation class IndexAllTasksServlet
 */
@WebServlet("/index/all_tasks")
public class IndexAllTasksServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public IndexAllTasksServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("user_name") == null || session.getAttribute("password") == null) {
			...
			try {
				...
				// 全てのTaskの取得
				String sql2 = "SELECT * FROM TASK WHERE USERID=? ORDER BY TASKID DESC";
			...
			request.getRequestDispatcher("/WEB-INF/app/task/index_all_task.jsp").forward(request, response);
		}
	}
